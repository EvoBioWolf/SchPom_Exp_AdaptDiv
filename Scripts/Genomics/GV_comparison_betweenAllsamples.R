...
#write.table(phaseIII_table_total_red, "temporal_table_red.txt", sep="\t", row.names = F)
#write.table(phaseIII_table_total_pink, "temporal_table_pink.txt", sep="\t", row.names = F)
...
phaseIII_table_total_both %>% 
  ...
  theme(panel.border = element_blank(),  
        ...
        axis.text.y = element_text(size=10, colour="black"), 
        strip.text.y = element_text(angle = 360, size=12)) +
ggsave(paste0("plots/22_6_distributionGVs_perGene_phaseIII_both.png"), width = 8, height = 4, dpi = 400)
