...
# setwd("C:/Users/Sergio/LRZ Sync+Share/pombe_phase_III/var_pvalue_realigned/filtered_masker/")
# setwd("C:/Users/sertu336/LRZ Sync+Share/pombe_phase_III/competitions/")

#### Ecologycal Selection - (Asexual Growth and Top/Bottom Sel.)

competition_data<-read.table("Aug2017_data_8reps_correct_parentPops.txt", T) %>% 
...
inference_m.evol<-competition_data %>% 
  filter(Time=="AG") %>% 
...
####  Mating
mating_data<-read.table("mating_first5reps_corrected.txt", T) %>% 
  mutate(counts=red+wt, 
         red=red+1, 
         wt=wt+1) %>% 
  filter(counts>500) %>% 
...
for (treatment in c("Allo","Para", "LM", "Sym")) {
  ...
  mycor <- rcorr(as.matrix(table_perTreatment_sim), type="pearson")
  Cor_M<-rbind(Cor_M, data.frame(mycor$r) %>% 
                 mutate(treatment=treatment, 
                        comp_var=row.names(mycor$r)))
  Cor_pvalues_M<-rbind(Cor_pvalues_M, data.frame(mycor$P) %>% 
                         mutate(treatment=treatment, 
                                comp_var=row.names(mycor$P)))
}


# # only for Red Populations:
# table_Cov_M_R <- Cov_M
# table_Cor_M_R <- Cor_M
# table_Cor_pvalues_M_R <- Cor_pvalues_M
# # only for pink Populations:
# table_Cov_M_P <- Cov_M
# table_Cor_M_P <- Cor_M
# table_Cor_pvalues_M_P <- Cor_pvalues_M
...
Cor_M %>% 
  ...
  full_join((Cor_pvalues_M %>% 
               ...
               filter((comp_var2=="N_growth" & comp_var %in% c("N_mating", "N_WTS", "N_WBS")) |
                        (comp_var2=="N_mating" & comp_var %in% c("N_WTS", "N_WBS")) |
                        (comp_var2=="N_WTS" & comp_var %in% c("N_WBS"))) %>%
               mutate(pvalue=ifelse(value<0.05, "*", "")) %>% 
               select(-value)), 
            by=c("treatment", "comp_var", "comp_var2")) %>% 
  #Using Bars and colours:
  ggplot(aes(treatment, value, fill=value)) +
  geom_bar(stat = "identity") +
  geom_text(aes(x=treatment, y=0, label=pvalue), size=4)+
  scale_y_continuous(limits = c(-1,1)) +
  # scale_fill_gradient2(low = "red", mid = "gray20", high = "blue", midpoint = 0,
  #                      space = "Lab",
  #                      na.value = "grey50", guide = "colourbar",
  #                      aesthetics = "fill",
  #                      limits = c(-1,1)) +
  # Using onlu black bars
  # ggplot(aes(treatment, value)) +
  # geom_bar(stat = "identity") +
  # scale_y_continuous(limits = c(-1,1)) +
  # ggplot(aes(treatment, factor("Cor"))) +
...
for (treatment in c("Allo","Para", "LM", "Sym")) {
  ...
  data.frame(mycor$r) %>% 
    mutate(treatment=treatment, 
           comp_var=row.names(mycor$r)) 
  mycor$r["N_growth","N_mating"]

  plot1<- table_perTreatment %>%
    ggplot(aes(x=N_growth, y=N_mating)) +
    geom_point(aes(colour=evolved)) +
    scale_colour_manual(values=c("red3","cornflowerblue")) +
    geom_smooth(method='lm',formula=y~x, alpha=0.3) +
    annotate("text",  x=Inf, y = Inf, 
             label = paste0("R2: ", format(round(mycor$r["N_growth","N_mating"], 2), nsmall = 2), 
                            "\n", "p-value:", 
                            format(round(mycor$P["N_growth","N_mating"], 3), nsmall = 3)), 
             vjust=1, hjust=1)+
    xlab("Normalized Asexual Growth")+
    ylab("Normalized Sexual Reproduction")+
    #ggtitle(treatment)+
    theme_classic() + 
    theme(panel.border = element_blank(), 
          panel.grid.major = element_line(colour = "grey70"), 
          ...
          axis.text.x = element_text(size=12, colour="black"), 
          axis.text.y = element_text(size=12, colour="black"))
  plot2<- table_perTreatment %>%
    ggplot(aes(x=N_growth, y=N_WBS)) +
    geom_point(aes(colour=evolved)) +
    scale_colour_manual(values=c("red3","cornflowerblue")) +
    geom_smooth(method='lm',formula=y~x, alpha=0.3) +
    annotate("text",  x=Inf, y = Inf, 
             label = paste0("R2: ", format(round(mycor$r["N_growth","N_WBS"], 2), nsmall = 2), 
                            "\n", "p-value:", 
                            format(round(mycor$P["N_growth","N_WBS"], 3), nsmall = 3)), 
             vjust=1, hjust=1)+
    xlab("Normalized Asexual Growth")+
    ylab("Normalized Ecol. Sel. Bottom")+
    #ggtitle(treatment)+
    theme_classic() + 
    theme(panel.border = element_blank(), 
          panel.grid.major = element_line(colour = "grey70"), 
          ...
          axis.text.x = element_text(size=12, colour="black"), 
          axis.text.y = element_text(size=12, colour="black"))
  plot3<- table_perTreatment %>%
    ggplot(aes(x=N_growth, y=N_WTS)) +
    geom_point(aes(colour=evolved)) +
    scale_colour_manual(values=c("red3","cornflowerblue")) +
    geom_smooth(method='lm',formula=y~x, alpha=0.3) +
    annotate("text",  x=Inf, y = Inf, 
             label = paste0("R2: ", format(round(mycor$r["N_growth","N_WTS"], 2), nsmall = 2), 
                            "\n", "p-value:", 
                            format(round(mycor$P["N_growth","N_WTS"], 3), nsmall = 3)), 
             vjust=1, hjust=1)+
    xlab("Normalized Asexual Growth")+
    ylab("Normalized Ecol. Sel. Top")+
    #ggtitle(treatment)+
    theme_classic() + 
    theme(panel.border = element_blank(), 
          panel.grid.major = element_line(colour = "grey70"), 
          ...
          axis.text.x = element_text(size=12, colour="black"), 
          axis.text.y = element_text(size=12, colour="black"))
  plot4<- table_perTreatment %>%
    ggplot(aes(x=N_mating, y=N_WTS)) +
    geom_point(aes(colour=evolved)) +
    scale_colour_manual(values=c("red3","cornflowerblue")) +
    geom_smooth(method='lm',formula=y~x, alpha=0.3) +
    annotate("text",  x=Inf, y = Inf, 
             label = paste0("R2: ", format(round(mycor$r["N_mating","N_WTS"], 2), nsmall = 2), 
                            "\n", "p-value:", 
                            format(round(mycor$P["N_mating","N_WTS"], 3), nsmall = 3)), 
             vjust=1, hjust=1)+
    xlab("Normalized Sexual Reproduction")+
    ylab("Normalized Ecol. Sel. Top")+
    #ggtitle(treatment)+
    theme_classic() + 
    theme(panel.border = element_blank(), 
          ...
          axis.text.x = element_text(size=12, colour="black"), 
          axis.text.y = element_text(size=12, colour="black"))
  plot5<- table_perTreatment %>%
    ggplot(aes(x=N_mating, y=N_WBS)) +
    geom_point(aes(colour=evolved)) +
    scale_colour_manual(values=c("red3","cornflowerblue")) +
    geom_smooth(method='lm',formula=y~x, alpha=0.3) +
    annotate("text",  x=Inf, y = Inf, 
             label = paste0("R2: ", format(round(mycor$r["N_mating","N_WBS"], 2), nsmall = 2), 
                            "\n", "p-value:", 
                            format(round(mycor$P["N_mating","N_WBS"], 3), nsmall = 3)), 
             vjust=1, hjust=1)+
    xlab("Normalized Sexual Reproduction")+
    ylab("Normalized Ecol. Sel. Bottom")+
    #ggtitle(treatment)+
    ...
    theme(panel.border = element_blank(), 
          ...
          axis.text.x = element_text(size=12, colour="black"), 
          axis.text.y = element_text(size=12, colour="black"))
  plot6<- table_perTreatment %>%
    ggplot(aes(x=N_WTS, y=N_WBS)) +
    geom_point(aes(colour=evolved)) +
    scale_colour_manual(values=c("red3","cornflowerblue")) +
    geom_smooth(method='lm',formula=y~x, alpha=0.3) +
    annotate("text",  x=Inf, y = Inf, 
             label = paste0("R2: ", format(round(mycor$r["N_WTS","N_WBS"], 2), nsmall = 2), 
                            "\n", "p-value:", 
                            format(round(mycor$P["N_WTS","N_WBS"], 3), nsmall = 3)), 
             vjust=1, hjust=1)+
    xlab("Normalized Ecol. Sel. Top")+
    ylab("Normalized Ecol. Sel. Bottom")+
    #ggtitle(treatment)+
    theme_classic() + 
    theme(panel.border = element_blank(), 
  ...
  plot_grid(plot1, 
            ...
            plot6,
            ncol = 3, nrow = 2) +
    ggsave(paste0("./new_analyses_plots/10_2_correlations_", Ade_used, 
                  "_", treatment, ".png"), width = 9, height = 6, dpi = 400)
}



## Adding pair populations in SYM
...
plot1<-table_perTreatment %>% 
  ggplot(aes(x=N_growth, y=N_mating)) +
  geom_point(aes(colour=evolved)) +
  geom_line(aes(group=Pop), alpha=0.5) +
  scale_colour_manual(values=c("red3","cornflowerblue")) +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ", format(round(mycor$r["N_growth","N_mating"], 2), nsmall = 2), 
...
plot_grid(plot1, 
          ...
          ncol = 3, nrow = 2) +
  ggsave(paste0("./new_analyses_plots/10_2_correlations_", Ade_used, 
                "_", treatment, "_pair.png"), width = 9, height = 6, dpi = 400)
...
for (i_val in c(1)) {
  ...
  table_perTreatment<-sim_phenotype %>% 
    ...
    select(-dif_median_relative_v, -log_median_m.evol, -log_median_relative_v, 
           -log_relativeWBS, -log_relativeWTS)
  table_perTreatment_sim<-table_perTreatment %>% select(N_mating, N_growth, N_WBS, N_WTS)
table_cor_sym_all<-table_perTreatment %>% 
  select(Fraction, Pop, N_growth) %>% 
  spread(Fraction, N_growth) %>% 
  mutate(B_T_Growth=B-T) %>% 
  select(-B, -T) %>% 
  left_join(table_perTreatment %>% 
              select(Fraction, Pop, N_mating) %>% 
              spread(Fraction, N_mating) %>% 
              mutate(B_T_Mating=B-T) %>% 
              select(-B, -T), by="Pop") 
table_cor_sym<-table_cor_sym_all %>% 
  ungroup() %>% 
  select(-Pop) %>% 
  as.matrix() %>% 
  rcorr(type="pearson") 
plot1<-table_cor_sym_all %>%
  ggplot(aes(x=B_T_Growth, y=B_T_Mating)) +
  geom_point() +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ", 
          format(round(table_cor_sym$r["B_T_Growth","B_T_Mating"], 2), nsmall = 2), "\n", 
          "p-value:", format(round(table_cor_sym$P["B_T_Growth","B_T_Mating"], 3), nsmall = 3)), 
           vjust=1, hjust=1)+
  xlab("Diff. B-T Normalized Asexual Growth")+
  ylab("Diff. B-T Normalized Sexual Rep.")+
  #ggtitle(treatment)+
  theme_classic() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_line(colour = "grey70"), 
...
table_cor_sym_all<-table_perTreatment %>% 
  ...
  select(-B, -T) %>% 
  left_join(table_perTreatment %>% 
              select(Fraction, Pop, N_WBS) %>% 
              spread(Fraction, N_WBS) %>% 
              mutate(B_T_WBS=B-T) %>% 
              select(-B, -T), by="Pop") 
table_cor_sym<- table_cor_sym_all %>% 
  ungroup() %>% 
  ...
  as.matrix() %>% 
  rcorr(type="pearson") 
plot2<-table_cor_sym_all %>%
  ggplot(aes(x=B_T_Growth, y=B_T_WBS)) +
  geom_point() +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ", 
    format(round(table_cor_sym$r["B_T_Growth","B_T_WBS"], 2), nsmall = 2), "\n", 
    "p-value:", format(round(table_cor_sym$P["B_T_Growth","B_T_WBS"], 3), nsmall = 3)), 
    vjust=1, hjust=1)+
  xlab("Diff. B-T Normalized Asexual Growth")+
  ylab("Diff. B-T Normalized E. Sel. Bottom")+
  #ggtitle(treatment)+
  theme_classic() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_line(colour = "grey70"), 
...
table_cor_sym_all<-table_perTreatment %>% 
  ...
  select(-B, -T) %>% 
  left_join(table_perTreatment %>% 
              select(Fraction, Pop, N_WTS) %>% 
              spread(Fraction, N_WTS) %>% 
              mutate(B_T_WTS=B-T) %>% 
              select(-B, -T), by="Pop") 
table_cor_sym<- table_cor_sym_all %>% 
  ungroup() %>% 
  ...
  as.matrix() %>% 
  rcorr(type="pearson") 
plot3<-table_cor_sym_all %>%
  ggplot(aes(x=B_T_Growth, y=B_T_WTS)) +
  geom_point() +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ", format(round(table_cor_sym$r["B_T_Growth","B_T_WTS"], 2), nsmall = 2), "\n", "p-value:", format(round(table_cor_sym$P["B_T_Growth","B_T_WTS"], 3), nsmall = 3)), 
           vjust=1, hjust=1)+
  xlab("Diff. B-T Normalized Asexual Growth")+
  ylab("Diff. B-T Normalized E. Sel. Top")+
  #ggtitle(treatment)+
  theme_classic() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_line(colour = "grey70"), 
...
table_cor_sym<- table_cor_sym_all %>% 
  ...
  as.matrix() %>% 
  rcorr(type="pearson") 
plot4<-table_cor_sym_all %>%
  ggplot(aes(x=B_T_Mating, y=B_T_WTS)) +
  geom_point() +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ", 
                          format(round(table_cor_sym$r["B_T_Mating","B_T_WTS"], 2), nsmall = 2), "\n", 
                          "p-value:", format(round(table_cor_sym$P["B_T_Mating","B_T_WTS"], 3), nsmall = 3)), 
           vjust=1, hjust=1)+
  xlab("Diff. B-T Normalized Sexual Rep.")+
  ylab("Diff. B-T Normalized E. Sel. Top")+
  #ggtitle(treatment)+
  theme_classic() + 
  theme(panel.border = element_blank(), 
...
table_cor_sym<- table_cor_sym_all %>% 
  ...
  as.matrix() %>% 
  rcorr(type="pearson") 
plot5<-table_cor_sym_all %>%
  ggplot(aes(x=B_T_Mating, y=B_T_WBS)) +
  geom_point() +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ",format(round(table_cor_sym$r["B_T_Mating","B_T_WBS"], 2), nsmall = 2), "\n", "p-value:", format(round(table_cor_sym$P["B_T_Mating","B_T_WBS"], 3), nsmall = 3)), 
           vjust=1, hjust=1)+
  xlab("Diff. B-T Normalized Sexual Rep.")+
  ylab("Diff. B-T Normalized E. Sel. Bottom")+
  #ggtitle(treatment)+
...
table_cor_sym<- table_cor_sym_all %>% 
  ...
  as.matrix() %>% 
  rcorr(type="pearson") 
plot6<-table_cor_sym_all %>%
  ggplot(aes(x=B_T_WTS, y=B_T_WBS)) +
  geom_point() +
  geom_smooth(method='lm',formula=y~x, alpha=0.3) +
  annotate("text",  x=Inf, y = Inf, 
           label = paste0("R2: ",format(round(table_cor_sym$r["B_T_WTS","B_T_WBS"], 2), nsmall = 2), "\n", "p-value:", format(round(table_cor_sym$P["B_T_WTS","B_T_WBS"], 3), nsmall = 3)), 
           vjust=1, hjust=1)+
  xlab("Diff. B-T Normalized E. Sel. Top")+
  ylab("Diff. B-T Normalized E. Sel. Bottom")+
  #ggtitle(treatment)+
  theme_classic() + 
  theme(panel.border = element_blank(), 
...
plot_grid(plot1, 
          ...
          plot6,
          ncol = 3, nrow = 2) +
  ggsave(paste0("./new_analyses_plots/10_3_diffPair_orrelations_", Ade_used, 
                "_", treatment, ".png"), width = 9, height = 6, dpi = 400)
}
...
# N_median_relative<-ppls::normalize.vector(sim_phenotype$log_median_relative_v)
# N_median_m.evol<-ppls::normalize.vector(sim_phenotype$median_m.evol)
